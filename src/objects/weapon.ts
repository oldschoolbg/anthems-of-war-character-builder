...
export class Weapon {
  ...
  AddProperty(weaponProperty: WeaponProperty, ...props: any[]): Weapon {
    if (!weaponProperty.MultipleAllowed && this.Properties.find((p: WeaponProperty) => p.Key === weaponProperty.Key)) {
      return this;
    }
...
export const TwoHandedAxeHammerSword = new Weapon('2 Handed Axe / Hammer / Sword', 1, 7)
...
export const Javelin = new Weapon('Javelin', 1, 5)
  .AddProperty(Ranged)
...
export const ThrowingKnife = new Weapon('Throwing Knife', 1, 4)
  .AddProperty(Ranged)
  .AddProperty(OneHanded)
